    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i=1:obj.M
                ...
                temp(:,1) = temp(:,1)+abs(PopDec(:,2)+0.5);
                temp(:,2) = temp(:,2)+abs(PopDec(:,2)-0.5);                
                PopObj(:,i) = min(temp,[],2);
            end
        end
...
%             temp = load('D:\mycode_matlab\PlatEMO_3.5\PlatEMO\PFPS\IDMPM2T1_e_PFPS.mat', 'PSS');
            ...
            Draw(L(1:size(L,1)/2,:),'-','Color',[1 .2 .2],'LineWidth',5);
            Draw(L(size(L,1)/2+1:end,:),'-','Color',[.2 .2 1],'LineWidth',5);
        end
        ...
        function DrawObj(obj,Population)
            ...
            temp = load('C:\Users\wjy\Desktop\PlatEMO-master\PlatEMO-master\PlatEMO\TruePFPS\poly-PSPF\poly_2.mat', 'PF');
            L = temp.PF;
            len = size(L,1);
