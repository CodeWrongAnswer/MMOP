...
%                 temp(:,4) = temp(:,4)+100.*((PopDec(:,3)-.6).^2+1-cos(4.*2.*pi.*(PopDec(:,3)-.6)));
        ...
        function R = GetOptimum(obj,N)
            temp = load('C:\Users\wjy\Desktop\PlatEMO-master\PlatEMO-master\PlatEMO\TruePFPS\IDMP-e\IDMPM3T4_e.mat', 'PSS');
            R = temp.PSS;
        end
...
%             Draw(Population.objs,'o','MarkerSize',4,'Marker','o','Markerfacecolor',[1 .2 .2],'Color',[1 .2 .2]);
            ...
            temp = load('C:\Users\wjy\Desktop\PlatEMO-master\PlatEMO-master\PlatEMO\TruePFPS\IDMP-e\IDMPM3T4_e.mat', 'PF');
            L = temp.PF;
            len = size(L,1);
            Draw(L(1:len/3,:),'.','Color',[1 .2 .2]);
            Draw(L(len/3+1:len/3*2,:),'.','Color',[.2 1 .2]);
            Draw(L(len/3*2+1:end,:),'.','Color',[.2 .2 1]);
