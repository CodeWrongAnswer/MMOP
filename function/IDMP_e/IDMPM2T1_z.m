    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            ...
            for i=1:obj.M
                ...
                local = [0.4 0.5 0.6];
                t1 = abs(PopDec(:,2)+repmat(local,N,1));
                t1 = min(t1,[],2);
                t2 = abs(PopDec(:,2)+repmat(-local,N,1));
                t2 = min(t2,[],2);
                temp(:,1) = temp(:,1)+1.*(t1).^1;
                temp(:,2) = temp(:,2)+obj.a.*(t2).^1;                
                PopObj(:,i) = min(temp,[],2);
            end
        end
        ...
        function R = GetOptimum(obj,N)
            x(:,1) = linspace(-0.6,-0.4,N/6)';
            x(:,2) = linspace(0.3,0.7,N/6)';
            R = [x(:,1),repmat(-0.4,size(x(:,1)));x(:,1),repmat(-0.5,size(x(:,1)));x(:,1),repmat(-0.6,size(x(:,1)));x(:,2),repmat(0.4,size(x(:,1)));x(:,2),repmat(0.5,size(x(:,1)));x(:,2),repmat(0.6,size(x(:,1)))];
        end
