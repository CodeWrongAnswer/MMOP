...
%             mesh(ax,X,Y,Z,'EdgeColor',[1 .2 .2],'FaceAlpha',0,'linewidth',1);
%             X = X+0.5*ones(size(X));
%             Y = Y+0.5*ones(size(Y));
%             Z = Z+0.5*ones(size(Z));
%             mesh(ax,X,Y,Z,'edgecolor',[.2 .2 1],'FaceAlpha',0,'linewidth',1);  
        end
